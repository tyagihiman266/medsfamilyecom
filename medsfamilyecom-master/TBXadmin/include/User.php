...

{
	...
	public function getResultById1($table, $id) {

		global $rowCount;

		$resultArray = array();

		$result = $this->db->query("Select  * from ".$table." where attribute_id = '".$id."'");
	...
	public function getResultById2($table, $id) {

		global $rowCount;

		$resultArray = array();

		$result = $this->db->query("Select  * from ".$table." where pcl_id = '".$id."'");
	...
	public function getResultById3($table, $id) {

		global $rowCount;

		$resultArray = array();

		$result = $this->db->query("Select  * from ".$table." where parent_typeid = '".$id."'");
	...
	public function getProductCatResultById($table, $id) {

		global $rowCount;

		$resultArray = array();

		$result = $this->db->query("Select  * from ".$table." where product_id = '".$id."'");
	...
	public function QueryRun1($query) {
	...
	public function updateStatus($table,$col,$val,$id) {
		...
		$query = "UPDATE $table SET $col=2 WHERE ".strtolower($table)."_id != ".$id;		
	...
	public function updateStatus1($table,$col,$val,$id) {

		global $dbError;

		$dbError = true;

		$query = "UPDATE $table SET $col='$val' WHERE users_id = ".$id;
	...
	public function updateQuery($colArray,$tablename,$id) {
		...
		$query = "UPDATE $tablename SET ".implode(",",$update)." WHERE id = ".$id;
		 ...
		 $last_id = $this->db->query($query) or die($this->db->error);
	...
	public function updateAdminQuery($colArray,$tablename,$id) {
	...
	public function QueryCommonDelete($query) {
	...
	public function DeleteProductFilter($table,$tab_id) {

		$query = "DELETE FROM $table WHERE product_id = ".$tab_id;
	...
	public function QueryDelete2($table,$tab_id) {

		$query = "DELETE FROM $table WHERE attribute_id = ".$tab_id;

		$this->db->query($query) or  die($this->db->error);

	}

	public function QueryDelete3($table,$tab_id) {

		$query = "DELETE FROM $table WHERE attribute_list_id = ".$tab_id;

		$this->db->query($query) or  die($this->db->error);

	}

	public function QueryDelete4($table,$tab_id,$type) {

		$query = "DELETE FROM $table WHERE pcl_id = ".$tab_id." and type =".$type;

		$this->db->query($query) or  die($this->db->error);

	}

		public function QueryDelete5($table,$tab_id,$type) {

		$query = "DELETE FROM $table WHERE parent_typeid = ".$tab_id." and type =".$type;
	...
	public function getName($id,$table) {
		...
		$query = "SELECT name FROM $table WHERE ".$table."_id = $id";
...
class exits_user{
	...
	public function check_user_exitDj($table,$email){

		$email = $this->dbfunc->escapeString($email);

		$query = "select ".strtolower($table)."_id from $table where email='$email' ";
...

{
...

    {
        ...
        {			
			...
			$subject="Forget Password for Survey";

            $from = 'info@nationproducts.in';
	...
	/* //Verify User email Account from verification link */

	public function verifyEmail($email,$table,$code)    {
			...
			$query = "SELECT ".strtolower($table)."_id from $table WHERE email='$email' and verify_code='$code'"; 
			...
			$count_row = count($result); 
				...
				$colArray['verify_code']="";

				$colArray['status']=1;	

				$this->dbfunc->updateQuery($colArray,$table,$result[0][strtolower($table)."_id"]);  
		...
		/*** Check if User exits or not with id pass***/

		public function checkExits($uid, $pass, $tablename)

		{   

			$query = "SELECT ".strtolower($tablename)."_id,password,salt from $tablename WHERE ".strtolower($tablename)."_id=$uid ";
			...
			if ($count_row == 1) {	
				...
				$hash = $this->dbfunc->checkhashSSHA($salt, $pass);
...
class frontUser{
	...
	public function regUser($name, $lname, $email, $mobile, $password,$refer_id,$cash,$company_name,$company_logo,$dj){

		$name = $this->dbfunc->escapeString($name);

		$lname = $this->dbfunc->escapeString($lname);
		...
		$refer_id = $this->dbfunc->escapeString($refer_id);

		$cash = $this->dbfunc->escapeString($cash);
		...
		$verify_code = rand().md5(rand().uniqid('', true));

		$result = $this->dbfunc->QueryInsert("INSERT INTO user(name, lname, email, mobile, password, salt, verify_code, join_date,company_name,company_logo)VALUES('$name', '$lname', '$email', '$mobile', '$encrypted_password', '$salt', '$verify_code', '$join_date', '$company_name', '$company_logo')");

    if($result){

		if($refer_id!=""){

			$getPrice = $this->dbfunc->getResult("select * from reward where status='1' order by reward_id desc limit 1");

			$r_table = "user";

			$sender_point = $getPrice[0]['sender_point'];

			$reciever_point = $getPrice[0]['reciever_point'];

			$validity = $getPrice[0]['validity'];

			/* //$_POST['referer_user_id'] = $refer_id; */

			$_POST['wallet_cash'] = $reciever_point;

			$_POST['wallet_val'] = date('Y-m-d',strtotime('+30 days',strtotime(date('Y-m-d'))));

			$columnsList = $this->dbfunc->getColumns($r_table);

			$columnValue = $this->dbfunc->getColumnValues($columnsList);

			$insertId = $this->dbfunc->updateQuery($columnValue,$r_table,$result);

			$_POST = "";

			$_POST['wallet_cash'] = $cash+$sender_point;
			...
			$insertId = $this->dbfunc->updateQuery($columnValue,$r_table,$refer_id);

			$this->dbfunc->QueryInsert("INSERT INTO wallet(referer_user_id, user_id, time, date, date_val, amount, created_date)VALUES('$refer_id', '$result', '".date('h:i:s')."', '".date('Y-m-d')."', '".date('Y-m-d',strtotime('+30 days',strtotime(date('Y-m-d'))))."', '$sender_point', '".date('Y-m-d h:i:s')."')");
		...
		$subject="Verify Email Account for Tune It Live";
		...
		$body="Hello , ".$name."
			...
			You have created account on tuneitlive.com.Please confirm your account to activate your Profile. If you created this account then just click the following link to Activate your Account, if not just ignore  this email,

			<br /><br />

			Click Following Link To Activate Your Account
		...
		$body.=	"<a href='http://tuneitlive.in/verify.php?email=$email&code=$verify_code'>Click Here to Activate your Account</a>
	...
	public function check_login($email, $password, $check)

	{
		...
		$query = "SELECT user_id,name,password,salt from user WHERE email='$email'  and status='1'";
	...
	/* ////user Auto Login Check via Cookie */
	...
	public function check_auto_login($email, $hash)    {
		...
		$query = "SELECT user_id,name,password,salt from user WHERE email='$email' and password='$hash' and status='1'";
	...
	public function getUserInfo($Uniqueid)   

	{

		$query = "SELECT * FROM users where UNIQUE_ID='$Uniqueid' LIMIT 1";  
