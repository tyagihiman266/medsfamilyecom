...
if(isset($_GET['ids'])){
  ...
  $sql = $objT->QueryDelete('product_varient',$deleteid);
  
  if($sql){
    $sms = "<p style='text-align:center;color:green;'>Varient deleted successfully</p>"; 
    header("refresh:1;url=manage_varient?p_id=".$_REQUEST['p_id']);
...
if(isset($_POST['delete'])) {
    ...
    for($i=0; $i < $id_count; $i++) {
        ...
        $sql = $objT->QueryDelete('product_varient',$id);
    }
    header("refresh:1;url=manage_varient?p_id=".$_REQUEST['p_id']); 
}
?>
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Manage Varient</li>
          </ol>
    </section>
     <section class="content">
      ...
      <a href="manage-customer.php?status=2"><span class="btn btn-warning btn-small">Blacklist</span></a> -->
          ...
          <a href="add_product_varient?p_id=<?php echo $_REQUEST['p_id'] ?>"><span class="btn btn-success btn-small">Add varient</span></a>
          ...
          <?php
                 ...
                 $row=$objT->getResult('select * from product_varient where product_id="'.$_REQUEST['p_id'].'" order by id desc');
        ...
        <td> <input type="checkbox" value="<?php echo $row[$j]['id']; ?>" name="data[]" id="data" title="Select All" /></td> 
                        <td><?php echo $i; ?></td>
                         <td><?php echo $row[$j]['varient']; ?>  <?php echo $row[$j]['varient_unit']; ?></td>
                         <td><img src="upload/product/varient/big/<?php echo $row[$j]['varient_img']; ?>" height="100" width="100"> </td>

             <td>
             <?php if($row[$j]['status']==1){ ?>
            <a href="manage_varient?tag=ProgarmActivateDeactivate&active=0&id=<?php echo $row[$j]['id'];?>" onClick="return confirm('Are you sure to do this action.');" title="Activate/Deactivate Service"><span class="label label-success">Active</span></a>
            <?php } elseif($row[$j]['status']==0) {?>
            
                       <a href="manage_varient?tag=ProgarmActivateDeactivate&active=1&id=<?php echo $row[$j]['id'];?>"  onClick="return confirm('Are you sure to do this action.');" title="Activate/Deactivate Service">
            ...
            <td><a href="add_product_varient?p_id=<?php echo $_REQUEST['p_id']; ?>&edit=<?php echo $row[$j]['id'];?>"><span class="btn btn-success btn-xs"><i class="fa fa-edit"></i>Edit</span></a><a href="manage_varient?p_id=<?php echo $_REQUEST['p_id']; ?>&ids=<?php echo $row[$j]['id'];?>" class="btn btn-danger btn-xs" onClick="return confirm('Are you sure want to delete')">Delete</a></td>
