...
if(isset($_GET['ids'])){
  ...
  $sql = $objT->QueryDelete('tbl_company',$deleteid);
  
  if($sql){
    $sms = "<p style='text-align:center;color:green;'>Company deleted successfully</p>"; 
    header("refresh:1;url=manage_company?p_id=".$_REQUEST['p_id']);
...
if(isset($_POST['delete'])) {
    ...
    for($i=0; $i < $id_count; $i++) {
        ...
        $sql = $objT->QueryDelete('tbl_company',$id);
    }
    header("refresh:1;url=manage_company?p_id=".$_REQUEST['p_id']); 
}
?>
    ...
    <div class="wrapper">
     ...
     <section class="content">
      ...
      <a href="manage-customer.php?status=2"><span class="btn btn-warning btn-small">Blacklist</span></a> -->
          ...
          <a href="add_product_company?p_id=<?php echo $_REQUEST['p_id'] ?>"><span class="btn btn-success btn-small">Add Company</span></a>
          ...
          <?php
                 ...
                 $row=$objT->getResult('select * from tbl_company where product_id="'.$_REQUEST['p_id'].'" order by id desc');
        ...
        <td> <input type="checkbox" value="<?php echo $row[$j]['id']; ?>" name="data[]" id="data" title="Select All" /></td> 
            ...
            <a href="manage_company?tag=ProgarmActivateDeactivate&active=0&id=<?php echo $row[$j]['id'];?>" onClick="return confirm('Are you sure to do this action.');" title="Activate/Deactivate Service"><span class="label label-success">Active</span></a>
            <?php } elseif($row[$j]['status']==0) {?>
            
                       <a href="manage_company?tag=ProgarmActivateDeactivate&active=1&id=<?php echo $row[$j]['id'];?>"  onClick="return confirm('Are you sure to do this action.');" title="Activate/Deactivate Service">
            ...
            <td><a href="add_product_company?p_id=<?php echo $_REQUEST['p_id']; ?>&edit=<?php echo $row[$j]['id'];?>"><span class="btn btn-success btn-xs"><i class="fa fa-edit"></i>Edit</span></a><a href="manage_company?p_id=<?php echo $_REQUEST['p_id']; ?>&ids=<?php echo $row[$j]['id'];?>" class="btn btn-danger btn-xs" onClick="return confirm('Are you sure want to delete')">Delete</a></td>
